...
public class ChatRoomsComposite extends ListExplanatoryComposite {
    ...
    static final Color WHITE = Display.getDefault()
    ...
    private CTabItem createChatTab(IChat chat) {
        ...
        try {
            control = new ChatControl(this, chat, chatRooms, SWT.BORDER, WHITE,
                2);
    ...
    public ChatControl getSelectedChatControl() {
        return !isChatExistent() ? null
            : (ChatControl) chatRooms.getSelection().getControl();
    }
    ...
    private IChat createChat(ISarosSession session) {
        MultiUserChatPreferences preferences = session.isHost()
