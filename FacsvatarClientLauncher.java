...
public class FacsvatarClientLauncher {
	...
	public FacsvatarClientLauncher(String ip_address) {
		...
		Runnable unity = new Runnable() {
			public void run() {
				try {
					String line;
					ProcessBuilder procBuild = new ProcessBuilder("FACSvatar_client_unity\\unity_FACSvatar.exe");
					procBuild.redirectErrorStream(true);
					process_unity = procBuild.start();
					BufferedReader input = new BufferedReader(new InputStreamReader(process_unity.getInputStream()));
		...
		Runnable facsvatar_pub_blend = new Runnable() {
			...
			public void run() {
				...
				String[] arguments = { "python", "FACsvatar_client_interface\\modules\\process_facstoblend\\pub_blend.py" };
				try {
					String line;
					...
					BufferedReader input = new BufferedReader(
							new InputStreamReader(process_pub_blend.getInputStream()));
					while ((line = input.readLine()) != null) {
						System.out.println(line);
					}
	...
	public static void main(String args[]) throws SocketException {
		...
		//FacsvatarServerLauncher server = new FacsvatarServerLauncher("192.168.0.108");
