// get ip from agent 
// openface, change xml to right ip
// start openface, user needs to open the webcam manually
...
class FacsvatarServerLauncher{
	...
	FacsvatarServerLauncher(String ip){
		...
		Runnable openface = new Runnable() {
			public void run() {
				try {
					String line;
					ProcessBuilder procBuild = new ProcessBuilder("run_openface.bat");
	...
	try {/*
		 ...
		 //needs correct file location to put the config into the openface directory
         StreamResult result = new StreamResult(new File("OpenFace_build\\config.xml"));
         transformer.transform(source, result);*/
		try (PrintWriter out = new PrintWriter("OpenFace_build\\config.xml")) {
		    out.println("<Config><Mode>push</Mode><IP>" + input_ip + "</IP><Port>5570</Port><Topic>openface</Topic></Config>");
		}
